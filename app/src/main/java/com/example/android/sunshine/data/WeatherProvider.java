...
//  DONE (8) Provide an implementation for the query method
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        // get access to the database (read-only)
        final SQLiteDatabase database = mOpenHelper.getReadableDatabase();

        // find the match code for the URI
...
//      DONE (9) Handle queries on both the weather and weather with date URI
        switch (match) {
            ...
            case CODE_WEATHER_WITH_DATE:
                // we have to look at the last path segment for the date. It represents the number of seconds since the epoch.
                // example: content://com.example.android.sunshine/weather/1472214172
                final String normalizedUtcDateString = uri.getLastPathSegment();

                // put the date in a string array for the selectionArgs
                final String[] ourSelectionArgs = new String[]{normalizedUtcDateString};

                returnCursor = database.query(WeatherContract.WeatherEntry.TABLE_NAME,
                                              projection,
                                              // we want to query an exact date which is specified in the URI
